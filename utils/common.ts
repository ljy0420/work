...
export const getGreeting = () => {
    ...
    if (hour < 5) {
        ...
        greet = i18n.global.t('utils.good morning!')
    } else if (hour < 12) {
        greet = i18n.global.t('utils.Good morning!')
    } else if (hour < 14) {
        greet = i18n.global.t('utils.Good noon!')
    } else if (hour < 18) {
        greet = i18n.global.t('utils.good afternoon')
    } else if (hour < 24) {
        greet = i18n.global.t('utils.Good evening')
    } else {
        greet = i18n.global.t('utils.Hello!')
    }
    return greet
}
...
/**
 * 多个不同表单校验 单表单则用自带的验证
 * @param formRefs 一个包含多个表单实例数组
 */
export const validateForm = (formRefs: FormInstance[]) => {
    return Promise.all(
