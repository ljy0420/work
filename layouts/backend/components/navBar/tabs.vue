    ...
    <div class="nav-tabs" ref="tabScrollbarRef">
        ...
        >
            ...
            <transition @after-leave="selectNavTab(tabsRefs[navTabs.state.activeIndex])" name="el-fade-in">
                <Icon v-show="navTabs.state.tabsView.length > 0 && item.name != 'dashboard'" class="close-icon" @click.stop="closeTab(item)" size="15" name="el-icon-Close" />
            </transition>
        </div>
...
const state: {
    ...
    contextmenuItems: [
        ...
        { name: 'closeOther', label: '关闭其他', icon: 'fa fa-minus' },
        { name: 'closeAll', label: '关闭全部', icon: 'fa fa-stop' },
    ],
})
