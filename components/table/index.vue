<template>
    <div>
        ...
        >
            ...
            <template #empty v-if="props.emptyShowAdd">
                <div>
                    <el-tooltip content="新增" placement="top">
                        <el-button link v-blur type="primary" size="small" @click="onAction('add')">
                            <Icon name="local-create" style="color: #59A7FF;width:27px;height:27px" class="local-icon" />
                        </el-button>
                    </el-tooltip>
                    <div>暂无数据</div>
                </div>
        ...
        <div v-if="props.pagination" class="table-pagination">
            <el-pagination
                ...
                :page-size="baTable.table.filter!.size"
                :page-sizes="pageSizes"
                :background="false"
                :layout="config.layout.shrink ? 'prev, next, jumper' : '->, total, sizes, prev, pager, next, jumper'"
...
const pageSizes = computed(() => {
    ...
    if (baTable.table.filter!.size) {
        if (!defaultSizes.includes(baTable.table.filter!.size)) {
            defaultSizes.push(baTable.table.filter!.size)
        }
    }
    return defaultSizes
})
