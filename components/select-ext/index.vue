        ...
        <template #empty>
            <div class="t-table-select__table" :style="{ width: `${tableWidth}px` }">
                <el-table ref="selectTable" :data="state.tableData" :class="{
                    ...
                    keyUpStyle: isKeyup,
                }" highlight-current-row :border="showborder" :row-key="getRowKey" @row-click="rowClick"
                    @cell-dblclick="cellDblclick" v-bind="$attrs"
                    :show-header="showheader">
                    <el-table-column type="radio" width="55" :label="radioTxt" fixed align="center"
                        v-if="isShowFirstColumn">
                        <template #default="scope">
                            <el-radio v-model="radioVal" :label="scope.$index + 1"
                                @click.stop="radioChangeHandle($event, scope.row, scope.$index + 1)"></el-radio>
                        </template>
                    </el-table-column>
                    <el-table-column v-for="(item, index) in columns" :key="index + 'i'" :type="item.type"
                        :label="item.label" :prop="item.prop" :width="item.width"
                        :min-width="item['min-width'] || item.minWidth" :align="item.align || 'center'" :fixed="item.fixed"
                        :show-overflow-tooltip="item.noShowTip" v-bind="{ ...item.bind, ...$attrs }">
                        <template #default="scope">
                            <!-- 作用域插槽 -->
                ...
                <div class="t-table-select__page" v-if="isShowPagination">
                    <el-pagination small background layout="prev, pager, next, total" :page-size="props.pageSize"
                        :total="props.total" @current-change="handlesCurrentChange" v-bind="$attrs" />
                </div>
            </div>
        </template>
    </el-select>
...
// 抛出事件
const emits = defineEmits(['page-change', 'radioChange', 'selectClear'])
// 当前页码
...
const defaultSelect = (defaultSelectVal: any) => {
    ...
    setTimeout(() => {
        ...
        if(defaultSelectVal) {
            selectTable.value.setCurrentRow(row)
            isRadio.value = true
        }
    }, 0)
}
...
const filterMethodHandle = (val: any) => {
    ...
    if (tableData && tableData.length > 0) {
        ...
        tableData.forEach((item) => {
            if (Object.hasOwn(item, props.keywords.pym) && !Object.hasOwn(item, props.keywords.wbm)) {
                if (item[props.keywords.label].includes(val) || item[props.keywords.pym].includes(val)) {
                    filterData.push(item)
                }
            } else if (!Object.hasOwn(item, props.keywords.pym) && Object.hasOwn(item, props.keywords.wbm)) {
                if (item[props.keywords.label].includes(val) || item[props.keywords.wbm].includes(val)) {
                    filterData.push(item)
                }
            } else if (Object.hasOwn(item, props.keywords.pym) && Object.hasOwn(item, props.keywords.wbm)) {
                if (item[props.keywords.label].includes(val) || item[props.keywords.pym].includes(val) || item[props.keywords.wbm].includes(val)) {
                    filterData.push(item)
                }
            } else {
                if (item[props.keywords.label].includes(val)) {
