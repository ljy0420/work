<template>
    ...
    <div>
        ...
        <Table :pagination="false" max-height="750px">
            <template #ysbmSlot>
                <el-table-column prop="YSBM" label="医生姓名" width="150" align="center">
                    <template #default="scope">
                        <!-- <span>{{ renderLx(scope.row.YSBM) }}</span> -->
                        <span>{{ scope.row.YSXM }}</span>
                    </template>
                </el-table-column>
            </template>
            <template #jggzSlot>
        ...
        <!-- 表单 -->
        <EditForm :ysData="state.ysData" :rowBm="prop.rowBm" />
    </div>
</template>

<script setup lang="ts">
import { provide, reactive, onMounted, watch, nextTick } from 'vue'
import baTableClass from '/@/utils/baTable2'
...
import { baTableApi2 } from '/@/api/common'
...
const baTable = new baTableClass(
    new baTableApi2('/api/v1/xtsjwh/ghlxwh/Cxghlxglry'),
    {
        expandAll: true,
        column: [
            { type: 'selection', align: 'center' },
            { label: '挂号类别编码', prop: 'LBBM', align: 'left', width: 130 },
            { label: '医生编码', prop: 'YSBM', align: 'center', render: 'slot', slotName: 'ysbmSlot' },
...
onMounted(() => {
    ...
    getCxmzys({})
        .then((res) => {
            if (res.code == 1) {
                state.ysData = res.data
                console.log('state.ysData', state.ysData)
            }
        })
        .catch((err) => {
            console.log(err)
