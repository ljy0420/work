<template>
    <!-- 对话框表单 -->
    <el-dialog
        class="ba-operate-dialog jbxx-edit"
    ...
    >
        <template #header>
            <div class="title" v-drag="['.ba-operate-dialog', '.el-dialog__header']" v-zoom="'.ba-operate-dialog'">
                {{ baTable.form.operate == 'Add' ? '疾病信息-新增' : '疾病信息-修改' }}
            </div>
        </template>
        <el-scrollbar v-loading="baTable.form.loading" class="ba-table-form-scrollbar">
            ...
            >
                ...
                >
                    <el-form-item prop="JBBM" label="疾病编码">
                        <el-input
                            v-model="baTable.form.items!.JBBM"
                            type="string"
                            placeholder="请输入疾病编码"
                        ></el-input>
                    </el-form-item>
                    <el-form-item prop="JBMC" label="疾病名称">
                        <el-input
                            v-model="baTable.form.items!.JBMC"
                            type="string"
                            @change="mChange"
                            placeholder="请输入疾病名称"
                        ></el-input>
                    </el-form-item>
                    <el-form-item prop="ICDM" label="ICD码">
                        <el-input
                            v-model="baTable.form.items!.ICDM"
                            type="string"
                            placeholder="请输入ICD码"
                        ></el-input>
                    </el-form-item>
                    <el-form-item prop="FLBM" label="疾病分类">
                        <el-select
                    ...
                    <el-form-item prop="JBLX" label="疾病类型">
                        <el-select
                        v-model="baTable.form.items!.JBLX"
                        placeholder="请选择疾病类型"
                        clearable
                        style="width: 100%;"
                        >
                            <el-option v-for="opt in jblxOptions" :key="opt.value" :label="opt.label" :value="opt.value"></el-option>
                        </el-select>
                    </el-form-item>
                    ...
                    <FormItem
                        label="是否传染病"
                        v-model="baTable.form.items!.SFCRB"
                        type="radio"
...
const rules: Partial<Record<string, FormItemRule[]>> = reactive({
    JBBM: [buildValidatorData({ name: 'required', title: '疾病编码不能为空.' })],
    JBMC: [buildValidatorData({ name: 'required', title: '疾病名称不能为空.' })]
})
