        ...
        <div class="yd-card-body">
            <el-table ref="tableref" :data="state.ykData" class="color-table"
                    ...
                    <el-table-column prop="YPMC" label="品名" align="center" width="170"
                        ...
                        <template #default="scope">
                            <SelectTable :disabled="examineTime != undefined" :agrs="scope.row" :data="ypInfo"
                                ...
                                :isShowPagination="false" :isKeyup="true" filterable @radioChange="singleSelectChange" />
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="SL" label="大单位数量" align="center" width="85">
                        <template #default="scope">
                            <el-input @change="slhandleChange(scope.row)" oninput="value=value.replace(/[^-?\d]/g,'')"
                                ...
                                @keydown.enter="handleKeyDown($event, scope, 'fysl-classl')"></el-input>
                        </template>
                    </el-table-column>
                    <el-table-column prop="SL" label="小单位数量" align="center" width="85">
                        <template #default="scope">
                            <el-input @change="slhandleChange(scope.row)" oninput="value=value.replace(/[^-?\d]/g,'')"
                                ...
                                :disabled="!scope.row.isEdit || examineTime != undefined" style="width: 100%"
                                @keydown.enter="handleKeyDown($event, scope, 'BZ')"></el-input>
                        </template>
                    </el-table-column>
                </el-table-column>
                ...
                <el-table-column prop="BZ" label="备注" align="center" width="150">
                    <template #default="{ row }">
                        <el-input v-model="row.BZ" type="string" :disabled="!row.isEdit || examineTime != undefined"
                            style="width: 100%" class="BZ"></el-input>
                    </template>
                </el-table-column>
...
const handleKeyDown = (event, scope, nextClass) => {
    if (event.keyCode === 13) {
        // 按下回车键聚焦到下一个输入框
        if (nextClass) {
...
const slhandleChange = (row) => {
    // if (row.DDWSL != 0 && row.DDWSL) {
    //     let SL = Number(row.DDWSL) * Number(row.YKBZ) + Number(row.XDWSL)
    //     console.log(typeof row.DDWSL)
    //     console.log(typeof row.XDWSL)
    //     console.log('dddddSL: ', SL)
    //     row.JHJE = (SL * Number(row.JHJ)).toFixed(2)
    //     row.LSJE = (SL * Number(row.LSJ)).toFixed(2)
    // } else {
    //     return
    // }
