<template>
    <div class="layout-box">
        ...
        <div class="search-box">
            <el-form :model="formData" :inline="true" label-width="auto" class="zw-table-search">
                <el-form-item label="药房" label-width="40">
                    <el-select v-model="formData.YFBM" placeholder="请选择药房" clearable @change="select" class="kf-select">
                ...
                <el-form-item>
                    <el-button type="primary" @click="getDcdsList">查询</el-button>
        ...
        <div class="table-item-top">
            ...
            <div class="export" @click="exportYkBtn" :class="[currentradio ? 'btn' : 'current-btn']">
        ...
        >
            ...
            <el-table-column prop="JSKSMC" label="接收科室" align="center" width="100"> </el-table-column>
            ...
            <el-table-column prop="SQR" label="申请人" align="center" width="100">
                <template #default="scope">
                    <span>{{ renderSQR(scope.row.SQR) }}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="GJZE" label="购进总额" align="center" width="120"> </el-table-column>
            <el-table-column prop="LSZE" label="零售总额" align="center" width="120"> </el-table-column>
            ...
            <el-table-column fixed="right" label="操作" align="center" width="200">
                <template #default="{ row }">
                    <div class="table-item">
                        <div class="btn" @click="dcdLokk(row)">
                            <Icon name="local-look" style="color: #4ca7f9" class="local-icon" />
                            <div>查看</div>
                        </div>
                        <div class="btn" v-if="row.ZT == 1" @click="dcdEdit(row)">
...
import { getgyslist, getyklist, getrylist } from '/@/api/common'
import { ElMessage, ElMessageBox } from 'element-plus'
import { useNavTabs } from '/@/stores/navTabs'
import { getDcdList, yfDcdEdit, cxYfYk } from '/@/api/backend/yfzx/dcd'
...
// 根据申请人编码获取申请人姓名
const renderSQR = (bm: string) => {
    let ret = bm
    ret = personnelIfon.value.find((item: any) => item.RYBM == bm)?.RYXM
    return ret
}

const handleRowClick = (row: any) => {
    // row.ID ? (currentradio.value = row.ID) : ''
...
// 调出单列表
const getDcdsList = async () => {
    const res = await getDcdList({ agrs: { ...formData } })
...
// 导出
const exportYkBtn = () => {
    export_table_to_excel('#exportTable', '调出单')
}
// 单据状态
const djztData = [
    { value: '0', MC: '已作废' },
...
const dcdCreate = () => {
    router.push({
        path: '/admin/yfzx/dcd/add',
        query: { cYFBM: formData.YFBM, type: 'create' },
    })
}
// 编辑调出单
const dcdEdit = (row: any) => {
    router.push({
        path: '/admin/yfzx/dcd/edit',
        query: { cYFBM: formData.YFBM, ID: row.ID, DJH: row.DJH, DJID: row.DJID, type: 'edit' },
    })
}
// 查看调出单
const dcdLokk = (row: any) => {
    router.push({
        path: '/admin/yfzx/dcd/detail',
        query: { cYFBM: formData.YFBM, ID: row.ID, DJH: row.DJH, DJID: row.DJID, type: 'look' },
    })
}
...
const loadOptions = async () => {
    ...
    if (res.data && res.data.length > 0) {
        ...
        if (lastTab.query.YFBM) {
            formData.YFBM = lastTab.query.YFBM.toString()
        }
    }
}
