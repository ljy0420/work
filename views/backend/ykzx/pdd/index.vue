<template>
    <div class="layout-box">
        ...
        <div class="table-item-top">
            ...
            <div class="btn" @click="exportYkBtn('export')">
        ...
        >
            ...
            <el-table-column prop="KFBM" label="盘点库房" align="center" width="160">
                <template #default="scope">
                    <span>{{ renderKfbm(scope.row.KFBM) }}</span>
                </template>
            </el-table-column>
...
import { getPddList, ykPddEdit, cxhwzd, hqyfyplb } from '/@/api/backend/yk/pdd'
...
const scpdMxSubmit = async () => {
    if (formData.YWLX == 1 || formData.YWLX == 2) {
        ...
        router.push({
            path: '/admin/ykzx/pdd/oder',
            query: { cKFBM: formData.KFBM, YWLX: formData.YWLX, type: 'oder' },
        })
    } else if (formData.YWLX == 3 || formData.YWLX == 4) {
        mxdialogVisible.value = false
        router.push({
            path: '/admin/ykzx/pdd/oder',
            query: { cKFBM: formData.KFBM, YWLX: formData.YWLX, ListBM: state.HWBM ? state.HWBM : state.LB, type: 'oder' },
        })
    } else if (formData.YWLX == 5) {
        mxdialogVisible.value = false
        router.push({
            path: '/admin/ykzx/pdd/oder',
            query: { cKFBM: formData.KFBM, YWLX: formData.YWLX, Date: formattedTime(), type: 'oder' },
        })
    } else {
...
watch(
    ...
    (val) => {
        if (val) {
            getPddsList()
            // 获取货位信息
            cxhwzd({ agrs: { KFBM: val, YWLX: 'Tree' } }).then((res) => {
                groupOptions.value = res.data
            })
            // 获取药品分类
            hqyfyplb({ agrs: formData.KFBM }).then((res) => {
