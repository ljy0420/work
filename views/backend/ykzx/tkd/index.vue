    ...
    <div class="layout-box">
        ...
        <div class="search-box">
            <el-form :model="formData" :inline="true" label-width="auto" class="zw-table-search">
                <el-form-item label="库房" label-width="40">
                    <el-select v-model="formData.KFBM" placeholder="请选择库房" @change="select" class="kf-select">
                ...
                <el-form-item>
                    <el-button type="primary" @click="getTkdsList">查询</el-button>
        ...
        <div class="table-item-top">
            <el-dropdown>
                ...
                <template #dropdown>
                    <el-dropdown-menu>
                        <el-dropdown-item @click="onCreate('1')">手工录入</el-dropdown-item>
                        <el-dropdown-item @click="onCreate('2')">根据出库单生成</el-dropdown-item>
                    </el-dropdown-menu>
                </template>
            </el-dropdown>
        ...
        >
            ...
            <el-table-column prop="TKKS" label="退库科室" align="left" width="90"> </el-table-column>
            ...
            <el-table-column prop="JSRXM" label="接收人" align="center" width="80"> </el-table-column>
            ...
            <el-table-column prop="CKDJH" label="引用出库单单据号" align="center" width="140"> </el-table-column>
            ...
            <el-table-column fixed="right" label="操作" align="center" width="200">
                <template #default="{ row }">
                    <div class="table-item">
                        <div class="btn" @click="onLook(row)">
                            <Icon name="local-look" style="color: #4ca7f9" class="local-icon" />
                            <div>查看</div>
                        </div>
                        <div class="btn" v-if="row.ZT == 1" @click="onEdit(row)">
...
import { getTkdList, ykTkdEdit, cxYfYk } from '/@/api/backend/yk/tkd'
...
const exportYkBtn = () => {
    export_table_to_excel('#exportTable', '退库单')
}
...
// 退库单列表
const getTkdsList = async () => {
    const res = await getTkdList({ agrs: { ...formData } })
...
const onEdit = (row: any) => {
    router.push({
        path: '/admin/ykzx/tkd/edit',
        query: { ID: row.ID, DJID: row.DJID, cKFBM: row.KFBM, DJH: row.DJH, JYLX: row.JYLX, type: 'edit' },
    })
}

const onCreate = (mode: string) => {
    router.push({
        path: '/admin/ykzx/tkd/add',
        query: { cKFBM: formData.KFBM, type: 'create', mode: mode },
    })
}
...
const loadOptions = async () => {
    ...
    if (res.data && res.data.length > 0) {
        ...
        if (lastTab.query.KFBM) {
            formData.KFBM = lastTab.query.KFBM.toString()
        }
    }
}
