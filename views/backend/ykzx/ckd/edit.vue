    ...
    <div class="layout-box">
            ...
            <el-scrollbar>
                <el-form :model="formData" :inline="true" label-width="100px" ref="ruleFormRef" :rules="rules">
                    <el-form-item prop="KDRQ" label="开单日期">
                        <el-date-picker v-model="formData.KDRQ" type="datetime" :disabled="examineTime != undefined"
                            placeholder="请选择开单日期" style="width: 100%" />
                    </el-form-item>
                    ...
                    <el-form-item label="请领部门" prop="KSBM">
                        <el-select filterable v-model="formData.KSBM" placeholder="请选择" :disabled="examineTime != undefined"
                            clearable style="width: 100%" class="QLBM" @keyup.enter="handleKeyDown5($event, 'QLR')">
                            <el-option v-for="opt in ykyfData" :key="opt.BM" :label="opt.MC" :value="opt.BM"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item prop="QLR" label="请领人">
                        <el-select v-model="formData.QLR" placeholder="请选择请领人" clearable style="width: 100%"
                            :disabled="examineTime != undefined" class="QLR" @keyup.enter="handleKeyDown4($event, 'YPMC')">
                            <el-option v-for="opt in personnelIfon" :key="opt.RYBM" :label="opt.RYXM"
                                :value="opt.RYBM"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="批准人">
                        <el-select v-model="formData.PZRBM" placeholder="请选择" :disabled="examineTime != undefined" clearable
                            style="width: 100%">
                            <el-option v-for="opt in personnelIfon" :key="opt.RYBM" :label="opt.RYXM"
                                :value="opt.RYBM"></el-option>
                        </el-select>
                    ...
                    <el-form-item label="备注">
                        <el-input v-model="formData.BZ" type="string" :disabled="examineTime != undefined"
                            placeholder="请输入备注信息" class="specs_frame" />
                    </el-form-item>
                </el-form>
            </el-scrollbar>
        </div>
                ...
                <el-table ref="tableref" :data="state.ykData" class="color-table"
                    ...
                    <el-table-column prop="name" label="药品" align="center" width="150">
                        ...
                        <el-table-column prop="YPMC" label="品名" align="center" width="170"
                            v-if="(receiptType == 'edit' || receiptType == 'create' || receiptType == 'order') && QLDJH == undefined">
                            <template #default="scope">
                                <SelectTable :disabled="examineTime != undefined" :agrs="scope.row" :data="ypInfo"
                                    :max-height="270" :tableWidth="800" :defaultSelectVal="scope.row.YPBM"
                                    :isShowQuery="false" :columns="[
                                        ...
                                        { label: '基本单位数量', prop: 'JBDWSL', width: '110' },
                                    ...
                                    @radioChange="singleSelectChange" class="YPMC"
                                    @blur="handleKeyDown2(scope, 'yk-FYSL')" />
                            </template>
                        </el-table-column>
                    ...
                    <el-table-column prop="DWBZ" label="包装" align="center" width="60">
                        <template #default="{ row }">
                            <span>{{ row.DWBZ }}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="CPPH" label="请领数量" align="center" width="80"
                        v-if="QLDJH != undefined || receiptType == 'qldOrder'">
                        <template #default="{ row }">
                            {{ row.QLSL }}
                        </template>
                    </el-table-column>
                    <el-table-column prop="CPPH" label="规格库存" align="center" width="80"
                        v-if="QLDJH != undefined || receiptType == 'qldOrder'">
                        <template #default="{ row }">
                            {{ row.GGKC }}
                        </template>
                    ...
                    <el-table-column prop="FYSL" label="库房数量" align="center" width="100">
                        <template #default="scope">
                            <el-form-item style="height: 15px" :prop="'ykData.' + scope.$index + '.FYSL'"
                                :rules="rules1.FYSL">
                                <el-input v-model.number="scope.row.FYSL" style="width: 100%"
                                    :disabled="!scope.row.isEdit || examineTime != undefined"
                                    oninput="value=value.replace(/[^\d]/g,'')" @change="slhandleChange(scope.row)"
                                    class="yk-FYSL"
                                    @keydown.enter="fyslhandleKeyDown($event, scope, 'yk-JBDWSL', 'yk-FYSL')"></el-input>
                            </el-form-item>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="JBDWSL" label="基本单位数量" align="center" width="100">
                        <template #default="scope">
                            <el-form-item style="height: 15px" :prop="'ykData.' + scope.$index + '.JBDWSL'"
                                :rules="rules1.JBDWSL">
                                <el-input v-model.number="scope.row.JBDWSL" style="width: 100%"
                                    :disabled="!scope.row.isEdit || examineTime != undefined"
                                    oninput="value=value.replace(/[^\d]/g,'')" @change="JBDWSLhandleChange(scope.row)"
                                    class="yk-JBDWSL"
                                    @keydown.enter="jbdwslhandleKeyDown($event, scope, 'BZ', 'yk-JBDWSL')"></el-input>
                            </el-form-item>
                        </template>
                    </el-table-column>

                    <el-table-column prop="JBDW" label="基本单位" align="center" width="70">
                        <template #default="{ row }">
                            <span>{{ row.JBDW }}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="" label="操作" width="150" fixed="right" align="center">
                        <template #default="scope">
                            <div style="display: flex; justify-content: space-around">
                                <el-button v-if="receiptType == 'order' || JHDJHs != undefined" link v-blur type="primary"
                                    size="small" :disabled="examineTime != undefined" round
                                    @click="addYk">
                                    新增
                                </el-button>
        ...
        <el-form :model="formData" :inline="true" label-width="90px" ref="ruleFormRefs" :rules="rules" class="bottom-form">
            <el-form-item label="单据类型" prop="DJLX">
                <el-select v-model="formData.DJLX" placeholder="请选择单据类型" :disabled="examineTime != undefined" clearable
                    style="width: 100%">
...
import { ykChdEdit, ykChdCreate, ykChdMx, getChdList, cxYfYk, dyJhdMx } from '/@/api/backend/yk/chd'
...
const prop = defineProps(['kfbm', 'jhdmxDetail', 'jhdjh', 'groupOption'])
...
const JBDWSLhandleChange = (row) => {
    row.FYSL = (row.JBDWSL / row.DWBZ).toFixed(3)
    slhandleChange(row)
}

const slhandleChange = (row) => {
    row.JHJE = (row.FYSL * row.JHJ).toFixed(2)
    row.LSJE = (row.LSJ * row.FYSL).toFixed(2)
    row.JBDWSL = (row.FYSL * row.DWBZ).toFixed(3)
}

const tableref = ref(null)
const fyslhandleKeyDown = (event: any, scope: any, nextClass: any, thisClass: any) => {
    if (event.keyCode === 13) {
        // 按下回车键聚焦到下一个输入框
        if (nextClass && scope.row.FYSL) {
...
const jbdwslhandleKeyDown = (event: any, scope: any, nextClass: any, thisClass: any) => {
    if (event.keyCode === 13) {
        // 按下回车键聚焦到下一个输入框
        if (nextClass && scope.row.JBDWSL) {
            const rowIndex = scope.$index
            let ele = tableref.value.$el.querySelector(`.el-table__row:nth-child(${rowIndex + 1})`).querySelector(`.${nextClass} .el-input__inner`)
            if (ele) {
                ele.focus()
...
const handleKeyDown3 = () => {
    let ele = ruleFormRef.value.$el.querySelector(`.QLBM .el-input__inner`)
    if (ele) {
        ele.focus()
    }
}
...
const getYpInfos = () => {
    getYpInfo({ agrs: { KFBM: KFBM, YWLX: '药库库存明细' } }).then((r: any) => {
        ypInfo.value = r.data
    })
}
getYpInfos()
...
const onSubmit = async () => {
    ...
    validateForm(formRefs).then(async (valid) => {
        if (valid.every((result) => result)) {
            const { ID, DJID, KFBM } = route.query
            ...
            if (receiptType == 'edit') {
                editdata = {
                    agrs: {
                        ZB: {
                            ...formData,
                            ID: ID || IDs.value,
                            DJID: DJID || DJIDs.value,
                            KFBM: KFBM || KFBMs.value,
                        },
                        MXB: validRows,
                        LX: '保存',
                    },
                }
                r = await ykChdEdit(editdata)
                ...
                createdata = {
                    agrs: {
                        ZB: { ...formData, KFBM: KFBM },
                        MXB: validRows,
                    },
                }
                r = await ykChdCreate(createdata)
                ...
                if (receiptType === 'order') {
                    createdata.agrs.ZB.JHDJH = JHDJH.value
                } else if (receiptType === 'qldOrder') {
                    createdata.agrs.ZB.QLDJH = JHDJH.value
                    createdata.agrs.ZB.QLKSBM = QLKSBM.value
                }
                r = await ykChdCreate(createdata)
                if (r.code === 1 && r.msg === '保存成功') {
                    receiptType = 'edit'
...
const examine = async () => {
    ...
    const handleExamine = async (data) => {
        ...
        const { code, data: responseData } = await ykChdEdit(data)
        if (code === 1) {
    ...
    if (ID) {
        const { DJID, KFBM, DJH, ID } = route.query
        const data = {
            agrs: {
                ZB: { DJID, KFBM, DJH, ID },
                LX: '审核',
...
const validateFYSL = (rule: any, value: any, callback: any) => {
...
const validateJBDWSL = (rule: any, value: any, callback: any) => {
    if (value === '' || value === 0 || value === undefined) {
        callback(new Error(''))
    } else {
        callback()
...
const rules1 = reactive({
    FYSL: [{ required: true, validator: validateFYSL, trigger: 'blur' }],
    JBDWSL: [{ required: true, validator: validateJBDWSL, trigger: 'blur' }],
})
const getlook = async () => {
...
.yk-FYSL :deep(.el-input__inner) {
    text-align: center;
}

.yk-JBDWSL :deep(.el-input__inner) {
    text-align: center;
}
</style>
